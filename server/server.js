...
function updateLastSeen(userInfo, longPoll) {
    ...
    if (found) {
        ...
        if (newUser.email) {
            newUser.gravatar = "http://www.gravatar.com/avatar/" + crypto.createHash('md5').update(userInfo.email).digest('hex')
        }

        wio.add(newUser).on('complete', function(data) {
          console.log("call frank to add '" + newUser.nickName + "': " + data); 
        });

        chatEvents.emit('broadcast', newUser);
    }
...
setInterval(function() {
    ...
    for(var ii=0; ii < currentUsers.length; ii++) {
        ...
        if(time + (timeout * 1000) < current) {
            ...
            wio.del(user.nickName).on('complete', function(data) {
              console.log("call frank to add '" + user.nickName + "': " + data); 
            });
        }
...
http.createServer(function(request, response) {
    ...
    switch(urlParts.pathname) {
        ...
        case '/message' :
            ...
            if (message.email) {
                message.gravatar = "http://www.gravatar.com/avatar/" + crypto.createHash('md5').update(message.email).digest('hex')
            }  
             
            chatEvents.emit('broadcast', message);
        ...
        default:
            fs.readFile(__dirname + '/../client' + urlParts.pathname, function(error, content) {
                if (error) {
                    ...
                    var extension = urlParts.pathname.slice(urlParts.pathname.lastIndexOf('.'));
                    var mime = 'text/plain';
                    switch(extension) {
                        case '.html' :
                            mime = 'text/html';
